...
- Granular module permissions (view/edit) and panel permissions (canView/canCreate/canEdit)
...
DEFAULT_ROLE_TEMPLATES = {
    "Admin": {
        "modules": {m: {"view": True, "edit": True} for m in SYSTEM_MODULES},
        "panels": {},
    },
    "Manager": {
        "modules": {m: {"view": True, "edit": m not in ("employees", "settings")} for m in SYSTEM_MODULES},
        "panels": {},
    },
    "Sales Executive": {
        "modules": {m: {"view": m in ("dashboard", "invoices", "quotations", "buyers", "reports"), "edit": m in ("invoices", "quotations", "buyers")} for m in SYSTEM_MODULES},
        "panels": {},
    },
    "Inventory Manager": {
        "modules": {m: {"view": m in ("dashboard", "inventory", "purchase_orders", "suppliers", "reports"), "edit": m in ("inventory", "purchase_orders", "suppliers")} for m in SYSTEM_MODULES},
        "panels": {},
    },
    "Accountant": {
        "modules": {m: {"view": m in ("dashboard", "invoices", "reports", "buyers"), "edit": m in ("invoices",)} for m in SYSTEM_MODULES},
        "panels": {},
    },
    "Viewer": {
        "modules": {m: {"view": True, "edit": False} for m in SYSTEM_MODULES},
        "panels": {},
    },
}
...
def init_employee_mgmt_router(db, verify_token_func, resolve_seller_id_func, sio=None):
    ...
    def build_perms_dict(data_perms: EmployeePermissions) -> dict:
        return {
            "modules": {k: {"view": v.view, "edit": v.edit} for k, v in data_perms.modules.items()},
            "panels": {k: {"canView": v.canView, "canCreate": v.canCreate, "canEdit": v.canEdit} for k, v in data_perms.panels.items()},
        }
    ...
    async def get_modules(authorization: str = Header(...)):
        ...
        modules = [{"id": m, "name": SYSTEM_MODULE_LABELS.get(m, m.replace("_", " ").title())} for m in SYSTEM_MODULES]
        panels = []
        if seller_id:
            raw = await db.panels.find({"sellerId": ObjectId(seller_id)}, {"_id": 1, "name": 1, "color": 1}).to_list(20)
            for p in raw:
                panels.append({"id": str(p["_id"]), "name": p.get("name", "Panel"), "color": p.get("color", "blue")})
    ...
    async def search_user_by_email(email: str, authorization: str = Header(...)):
        user = await get_current_user(authorization)
        seller_id = await get_seller_id(user)
        found = await db.users.find_one({"email": email.lower().strip()}, {"_id": 1, "email": 1, "name": 1, "phone": 1, "accountType": 1, "companyId": 1, "employeeStatus": 1, "profile": 1})
        if not found:
        ...
        return {"found": True, "canLink": True, "user": {"id": str(found["_id"]), "email": found.get("email", ""), "name": name, "phone": phone, "accountType": found.get("accountType", ""), "employeeStatus": found.get("employeeStatus", "pending")}}
    ...
    async def link_employee(data: LinkEmployeeRequest, authorization: str = Header(...)):
        ...
        update = {"companyId": ObjectId(seller_id), "employeeRole": data.role, "employeeStatus": "active", "employeePermissions": perms, "linkedAt": now, "updatedAt": now}
    ...
    async def list_employees_enhanced(tab: str = "active", authorization: str = Header(...)):
        ...
        if tab == "active":
            ...
            query = {"$or": [{"companyId": {"$exists": False}}, {"companyId": None}], "employeeStatus": {"$nin": ["active", "disabled", "unlinked"]}, "accountType": {"$nin": ["seller", "admin"]}, "unlinkedFrom": {"$exists": False}}
        elif tab == "unlinked":
        ...
        cursor = db.users.find(query, {"_id": 1, "email": 1, "name": 1, "phone": 1, "profile": 1, "employeeRole": 1, "employeeStatus": 1, "employeePermissions": 1, "linkedAt": 1, "unlinkedAt": 1, "accountType": 1, "createdAt": 1}).sort("updatedAt", -1).limit(100)
        docs = await cursor.to_list(100)
        employees = []
        for d in docs:
            profile = d.get("profile", {})
            employees.append({"id": str(d["_id"]), "email": d.get("email", ""), "name": d.get("name") or profile.get("businessName") or profile.get("name") or "", "phone": d.get("phone") or profile.get("phone") or "", "role": d.get("employeeRole", "unassigned"), "status": d.get("employeeStatus", "pending"), "permissions": normalize_permissions(d.get("employeePermissions", {})), "linkedAt": d.get("linkedAt", ""), "unlinkedAt": d.get("unlinkedAt", ""), "createdAt": d.get("createdAt", "")})
    ...
    async def update_employee_access(employee_id: str, data: UpdateEmployeeAccessRequest, authorization: str = Header(...)):
        ...
        await emit_access_update(employee_id, {"type": "access_updated", "role": data.role or emp.get("employeeRole"), "permissions": update_fields.get("employeePermissions", emp.get("employeePermissions", {})), "status": data.status or emp.get("employeeStatus")})
    ...
    async def unlink_employee(employee_id: str, authorization: str = Header(...)):
        ...
        if not emp:
            raise HTTPException(status_code=404, detail="Employee not found in your company")
        now = datetime.now(timezone.utc)
        await db.users.update_one({"_id": ObjectId(employee_id)}, {"$set": {"companyId": None, "employeeRole": "unassigned", "employeeStatus": "unlinked", "employeePermissions": {"modules": {}, "panels": {}}, "unlinkedFrom": ObjectId(seller_id), "unlinkedAt": now, "updatedAt": now}})
    ...
    async def relink_employee(employee_id: str, data: LinkEmployeeRequest, authorization: str = Header(...)):
        ...
        perms = build_perms_dict(data.permissions)
        now = datetime.now(timezone.utc)
        await db.users.update_one({"_id": ObjectId(employee_id)}, {"$set": {"companyId": ObjectId(seller_id), "employeeRole": data.role, "employeeStatus": "active", "employeePermissions": perms, "linkedAt": now, "updatedAt": now}, "$unset": {"unlinkedFrom": "", "unlinkedAt": ""}})
    ...
    async def get_my_access(authorization: str = Header(...)):
        user = await get_current_user(authorization)
        uid = str(user["_id"])
        doc = await db.users.find_one({"_id": user["_id"]}, {"_id": 0, "employeeRole": 1, "employeeStatus": 1, "employeePermissions": 1, "companyId": 1, "accountType": 1, "roles": 1, "profile": 1})
        if not doc:
            return {"role": "unassigned", "status": "pending", "permissions": {"modules": {}, "panels": {}}, "isAdmin": False, "permittedPanels": []}
        is_admin = doc.get("accountType") == "seller" or "seller" in (doc.get("roles") or [])
        perms = normalize_permissions(doc.get("employeePermissions", {}))
        company_name = ""
        company_logo_url = ""
        ...
        if is_admin:
            ...
            seller = await db.users.find_one({"_id": company_id}, {"_id": 0, "profile.businessName": 1, "profile.sellerLogoUrl": 1, "billingSettings.companyLogoUrl": 1})
            if seller:
            ...
            for pid, pp in perms["panels"].items():
                if isinstance(pp, dict) and pp.get("canView"):
                    try:
                        ...
                        pass
            if panel_ids:
                raw = await db.panels.find({"_id": {"$in": panel_ids}}, {"_id": 1, "name": 1, "color": 1, "slug": 1}).to_list(20)
                for p in raw:
                    permitted_panels.append({"id": str(p["_id"]), "name": p["name"], "color": p.get("color", "blue"), "slug": p.get("slug", "")})
        return {"userId": uid, "role": doc.get("employeeRole", "unassigned"), "status": doc.get("employeeStatus", "pending"), "permissions": perms, "companyId": str(doc["companyId"]) if doc.get("companyId") else None, "isAdmin": is_admin, "companyName": company_name, "companyLogoUrl": company_logo_url, "permittedPanels": permitted_panels}

    @router.get("/employee-mgmt/logs")
    async def get_employee_logs(authorization: str = Header(...), limit: int = 50):
        user = await get_current_user(authorization)
        seller_id = await get_seller_id(user)
        cursor = db.employee_logs.find({"sellerId": ObjectId(seller_id)}, {"_id": 0, "performedBy": 1, "action": 1, "targetUserId": 1, "details": 1, "timestamp": 1}).sort("timestamp", -1).limit(limit)
    ...
    async def unlink_all_employees(authorization: str = Header(...)):
        ...
        seller_id = await get_seller_id(admin)
        now = datetime.now(timezone.utc)
        employees = await db.users.find({"companyId": ObjectId(seller_id), "employeeStatus": {"$in": ["active", "disabled"]}}, {"_id": 1}).to_list(500)
        if employees:
            emp_ids = [e["_id"] for e in employees]
            await db.users.update_many({"_id": {"$in": emp_ids}}, {"$set": {"companyId": None, "employeeRole": "unassigned", "employeeStatus": "unlinked", "employeePermissions": {"modules": {}, "panels": {}}, "unlinkedFrom": ObjectId(seller_id), "unlinkedAt": now, "updatedAt": now}})
