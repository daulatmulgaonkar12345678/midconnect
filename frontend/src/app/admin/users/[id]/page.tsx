  ...
  async function handleBadgeUpdate(newBadgeType: 'none' | 'choice' | 'trusted') {
    try {
      ...
      const response = await fetchWithAuth<{ success: boolean; message: string }>(
        ...
        {
          method: 'PUT',
          body: { badgeType: newBadgeType }  // Pass object, not JSON string
        }
      );
    ...
    <div data-testid="user-detail-page">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-1 space-y-6">
          ...
          <div className="bg-white rounded-xl p-6 shadow-sm border border-gray-100" data-testid="badge-management-card">
            <h2 className="text-lg font-semibold text-gray-900 mb-4 flex items-center gap-2">
            ...
            <p className="text-sm text-gray-500 mb-4">
              Assign a trust badge to highlight this seller on their product listings.
            </p>
            
            {/* Current Badge Display */}
            <div className="mb-4">
              <span className="text-sm text-gray-500">Current Badge:</span>
              <div className="mt-2">
                ...
                {user.badgeType === 'choice' && (
                  <span className="inline-flex items-center gap-1.5 px-3 py-1.5 bg-yellow-100 text-yellow-800 text-sm font-semibold rounded-full border border-yellow-300">
                    <Star className="h-4 w-4 fill-yellow-500 text-yellow-500" />
                    UdyogConnect Choice
                  </span>
                )}
                {user.badgeType === 'trusted' && (
                  <span className="inline-flex items-center gap-1.5 px-3 py-1.5 bg-green-100 text-green-800 text-sm font-semibold rounded-full border border-green-300">
            ...
            {/* Badge Selector */}
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-2">Change Badge</label>
              <div className="flex items-center gap-3">
                <select
                  value={user.badgeType || 'none'}
                  onChange={(e) => handleBadgeUpdate(e.target.value as 'none' | 'choice' | 'trusted')}
                  disabled={badgeUpdating}
                  className="flex-1 px-3 py-2.5 border border-gray-200 rounded-lg text-sm focus:ring-2 focus:ring-blue-500 focus:border-blue-500 disabled:opacity-50 disabled:cursor-not-allowed"
                  data-testid="badge-select"
                >
                  <option value="none">No Badge</option>
                  <option value="choice">⭐ UdyogConnect Choice</option>
                  <option value="trusted">🛡️ UdyogConnect Trusted</option>
                </select>
                {badgeUpdating && (
              ...
              <p className="mt-2 text-xs text-gray-400">
                Badge will appear on all product listings by this seller.
              </p>
            </div>
          </div>
        </div>
