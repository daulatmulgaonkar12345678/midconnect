...
import {
  ...
  AlertCircle, Check, ChevronRight, ExternalLink
} from 'lucide-react';
...
export default function NotificationsPage() {
  ...
  if (!hasPermission('create_invoice')) {
    return <div className="text-center py-12 bg-white rounded-xl border" data-testid="no-permission"><p className="text-gray-500">No permission.</p></div>;
  }
    ...
    <div className="space-y-5" data-testid="notifications-page">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-1">{unread > 0 ? `${unread} unread` : 'All caught up'} — {total} total</p>
        </div>
        {unread > 0 && (
          <button onClick={markAllRead}
            className="flex items-center gap-1.5 px-3 py-2 text-sm font-medium text-indigo-600 border border-indigo-200 rounded-lg hover:bg-indigo-50" data-testid="mark-all-read-btn">
      ...
      {loading ? (
        ...
        <div className="text-center py-16 bg-white rounded-xl border border-gray-100" data-testid="empty-notifications">
          ...
          <p className="text-sm text-gray-400 mt-1">You&apos;ll see payment alerts and reminders here</p>
        </div>
      ) : (
          ...
          {notifications.map(n => {
            ...
            return (
              <div key={n.id}
                ...
                className={`flex items-start gap-3 p-4 rounded-xl border cursor-pointer transition ${n.read ? 'bg-white border-gray-100 hover:bg-gray-50' : 'bg-indigo-50/30 border-indigo-100 hover:bg-indigo-50/50'}`}
