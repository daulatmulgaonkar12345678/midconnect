...
import { BarChart3, TrendingUp, Package, Users, Calendar, Filter, IndianRupee } from "lucide-react";

const API_URL = process.env.NEXT_PUBLIC_API_URL;

type Tab = "sales" | "products" | "inventory" | "buyers";
      ...
      <div className="flex gap-1 bg-gray-100 p-1 rounded-lg w-fit">
        {tabs.map(t => (
          <button key={t.key} onClick={() => setTab(t.key)}
            className={`flex items-center gap-1.5 px-4 py-2 rounded-md text-sm font-medium transition-colors ${tab === t.key ? "bg-white text-indigo-600 shadow-sm" : "text-gray-500 hover:text-gray-700"}`}
