...
export default function CompleteProfilePage() {
  ...
  const [formData, setFormData] = useState({ businessName: '', phone: '', address: '', city: '', state: '', pincode: '', gstNumber: '' });
  ...
  const validateProfile = () => {
    ...
    if (selectedRole === 'seller') {
      ...
      if (!/^[0-9]{2}[A-Z]{5}[0-9]{4}[A-Z]{1}[1-9A-Z]{1}Z[0-9A-Z]{1}$/.test(formData.gstNumber)) { setLocalError('Please enter a valid GST number'); return false; }
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const profileData: ProfileCompleteData = { role: selectedRole, businessName: formData.businessName.trim(), phone: formData.phone, address: formData.address.trim(), city: formData.city.trim(), state: formData.state, pincode: formData.pincode, ...(selectedRole === 'seller' && { gstNumber: formData.gstNumber }) };
  ...
  if (!user || !emailVerified || (!needsRegistration && isAuthenticated)) return <div className="min-h-[80vh] flex items-center justify-center"><div className="animate-spin rounded-full h-8 w-8 border-b-2 border-blue-600" /></div>;
      ...
      <div className="w-full max-w-lg">
        <div className="bg-white rounded-xl shadow-lg p-8">
          <div className="text-center mb-8">
            ...
            <h1 className="text-2xl font-bold text-gray-900">{step === 'role' ? 'Choose Account Type' : 'Complete Your Profile'}</h1>
            <p className="text-gray-500 mt-2">{step === 'role' ? 'Select how you want to use the platform' : selectedRole === 'seller' ? 'Fill in your business details and GST information' : 'Fill in your business details'}</p>
          </div>

          {displayError && <div className="bg-red-50 border border-red-200 text-red-600 px-4 py-3 rounded-lg text-sm flex items-start gap-2 mb-6"><AlertCircle className="h-5 w-5 flex-shrink-0 mt-0.5" /><span>{displayError}</span></div>}

          {step === 'role' && (
            <div className="space-y-4">
              <button onClick={() => handleRoleSelect('buyer')} className="w-full p-6 border-2 border-gray-200 rounded-xl hover:border-blue-500 hover:bg-blue-50 transition group text-left" data-testid="role-buyer-btn">
                <div className="flex items-start gap-4">
                  <div className="w-12 h-12 bg-blue-100 rounded-lg flex items-center justify-center group-hover:bg-blue-200 transition"><User className="h-6 w-6 text-blue-600" /></div>
                  <div><h3 className="text-lg font-semibold text-gray-900">Buyer</h3><p className="text-gray-500 text-sm mt-1">Browse products, send inquiries, and connect with sellers</p></div>
                </div>
              </button>
              <button onClick={() => handleRoleSelect('seller')} className="w-full p-6 border-2 border-gray-200 rounded-xl hover:border-green-500 hover:bg-green-50 transition group text-left" data-testid="role-seller-btn">
                <div className="flex items-start gap-4">
                  <div className="w-12 h-12 bg-green-100 rounded-lg flex items-center justify-center group-hover:bg-green-200 transition"><Store className="h-6 w-6 text-green-600" /></div>
                  <div><h3 className="text-lg font-semibold text-gray-900">Seller</h3><p className="text-gray-500 text-sm mt-1">List your products, receive inquiries, and grow your business</p>
                    <p className="text-xs text-amber-600 mt-2 flex items-center gap-1"><Building2 className="h-3 w-3" /> Requires GST registration</p></div>
                </div>
              </button>
            </div>
          )}
          ...
          {step === 'profile' && selectedRole && (
            <form onSubmit={handleSubmit} className="space-y-5">
              <div className={`inline-flex items-center gap-2 px-3 py-1.5 rounded-full text-sm font-medium ${selectedRole === 'seller' ? 'bg-green-100 text-green-700' : 'bg-blue-100 text-blue-700'}`}>
                {selectedRole === 'seller' ? <Store className="h-4 w-4" /> : <User className="h-4 w-4" />}{selectedRole === 'seller' ? 'Seller Account' : 'Buyer Account'}
              </div>
              ...
              <div className="flex gap-3 pt-2">
                <button type="button" onClick={() => { setStep('role'); setSelectedRole(null); setLocalError(''); }} className="flex-1 border border-gray-300 text-gray-700 py-3 rounded-lg hover:bg-gray-50 transition">Back</button>
                <button type="submit" disabled={isSubmitting || authLoading} className="flex-1 bg-blue-600 text-white py-3 rounded-lg hover:bg-blue-700 transition flex items-center justify-center gap-2 disabled:opacity-50" data-testid="submit-profile-btn">{isSubmitting ? <div className="animate-spin rounded-full h-5 w-5 border-b-2 border-white" /> : 'Complete Registration'}</button>
              </div>
            </form>
          )}
        </div>
