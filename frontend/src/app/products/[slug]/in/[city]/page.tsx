...
export async function generateMetadata({ params }: CityPageProps): Promise<Metadata> {
  ...
  try {
    const res = await fetch(`${API}/api/products/${slug}/city/${city}`, { next: { revalidate: 3600 } });
    if (!res.ok) return { title: 'Product Not Found | UdyogConnect' };
          ...
          <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
            {sellers.map((seller: any, i: number) => (
              <div key={i} className="border rounded-lg p-4 hover:shadow-md transition bg-white" data-testid={`city-seller-${i}`}>
                <h3 className="font-semibold text-gray-900">{seller.businessName || 'Verified Supplier'}</h3>
